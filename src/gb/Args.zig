...
pub const Args = struct {
    ...
    pub fn parse_args() !Args {
        const params = comptime [_]clap.Param(clap.Help){
            ...
            clap.parseParam("-s, --string <str>     ROM filename") catch unreachable,
        };
        ...
        // const rom_path: []const u8 = try std.heap.page_allocator.dupe(u8, &res.args.string.?);
        // _ = rom_path; // autofix

        // if (res.args.string) |pos| {
        //     std.debug.print("Type of {}\n", .{@TypeOf(pos)});
        //     std.debug.print("Slice is {s}\n", .{pos});
        //     std.mem.copyForwards(u8, res.args.string.?, Self.rom);
        // }

        // const rom: ?[]const u8 = for (res.args.string) |pos| {
        //     if (pos != null) break pos;
        // } else null;
